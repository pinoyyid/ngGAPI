...
var MaximalCtrl = (function () {
    ...
    MaximalCtrl.prototype.doEverything = function () {
        ...
        this.getCountandDelete_xxxparanoid().then(function () {
            return _this.createFolder('paranoid');
        }).then(function (resp) {
            console.log(resp.id);
            return _this.insertFiles('xxxparanoid', 100, resp.id);
        }, function (reason) {
            console.error('incomplete insert, reason = ' + reason);
        }).then(function () {
            return _this.getCountandDelete_xxxparanoid();
        }, function (reason) {
            console.error('incomplete insert, reason = ' + reason);
        }).then(function () {
    ...
    MaximalCtrl.prototype.getCountandDelete_xxxparanoid = function () {
        ...
        var ro;
        ro = this.DriveService.files.list({ maxResults: 500, q: "title contains 'xxxparanoid'", fields: 'items/id, items/parents/id' });
