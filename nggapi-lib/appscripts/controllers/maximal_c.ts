...
class MaximalCtrl {
	...
	doEverything() {
		this.insertFiles('delmexxx', 2)
			.then (() => {return this.getFile(this.currentFile.id)})
			.then (() => {return this.getFileContents(this.currentFile.id)})
			.then (() => {return this.patchFileTitle(this.currentFile.id, this.currentFile.title+" PATCHED")})
			.then (() => {return this.updateFileTitle(this.currentFile.id, this.currentFile.title+" UPDATED")})
			.then (() => {return this.updateFileContent(this.currentFile.id, 'updated file content')})
			.then (() => {return this.touchFile(this.currentFile.id)})
			.then (() => {return this.trashFile(this.currentFile.id)})
